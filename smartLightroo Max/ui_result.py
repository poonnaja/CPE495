...
from database import save_all, log_activity, get_course_by_id
...
def render_result(is_admin: bool, actor: str | None, actor_role: str, chosen, new_proj: bool):
    ...
    # Save button + realtime auto-save (deduplicated by state signature)
    save_clicked = st.button("บันทึก", use_container_width=True, type="primary")
    should_auto_save = chosen is not None
    state_signature = "|".join([
        str(mode),
        str(sc.get("mode")),
        str(sc.get("lux")),
        str(sc.get("people")),
        str(sc.get("brightness_pct")),
        str(int(new_proj)),
        str(sc.get("hour")),
        str(sc.get("teacher_name")),
        str(sc.get("course_id")),
    ])
    already_saved = st.session_state.get("last_saved_signature") == state_signature

    if save_clicked or (should_auto_save and not already_saved):
    ...
    elif should_auto_save and already_saved:
        st.caption("สถานะปัจจุบันถูกบันทึกแล้ว")